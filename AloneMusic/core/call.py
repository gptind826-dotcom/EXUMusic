...
class Call:
    ...
    async def _ensure_assistant_ready(self, chat_id: int, assistant) -> None:
        """Auto-invite the assistant to the chat and promote it as admin."""
        pairs = [
        ...
        userbot = None
        for pytgc, pyrog in pairs:
            if assistant is pytgc:
                userbot = pyrog
                break
        if not userbot:
            return
        ...
        # 1 — make sure the assistant is a member of the chat
        try:
            await userbot.get_chat_member(chat_id, assistant_id)
        except Exception:
            try:
                await app.add_chat_members(chat_id, assistant_id)
                LOGGER(__name__).info(f"Auto-invited assistant {assistant_id} to {chat_id}")
            except UserAlreadyParticipant:
                pass
            except Exception as e:
                LOGGER(__name__).warning(f"Could not invite assistant to {chat_id}: {e}")

        # 2 — promote the assistant as admin with voice-chat permissions
        try:
            ...
            LOGGER(__name__).info(f"Auto-promoted assistant {assistant_id} in {chat_id}")
        except Exception as e:
            LOGGER(__name__).warning(f"Could not promote assistant in {chat_id}: {e}")
    ...
    async def join_call(
        ...
        # Auto-invite & promote assistant before joining the voice chat
        await self._ensure_assistant_ready(chat_id, assistant)
